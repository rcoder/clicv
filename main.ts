...
const injectSession = format((info, _) => ({ ...info, session: sessionId }));

const log = createLogger({
  transports: [new transports.File({ filename: 'clicv.log' })],
  format: format.combine(format.json(), injectSession())
});
...
allPages.forEach(page =>
  cli.command(page.name, chalk.greenBright(page.title)).action(async () => {
...
cli
  .command('contact', chalk.yellow('Send email to Lennon'))
  .action(async args => {
    ...
    const choices = await inquirer.prompt([
      {
        ...
        message: 'From:',
        validate: input =>
          input.match(emailPat) ? true : 'please enter an email address'
      },
      {
        type: 'suggest',
        name: 'subject',
        ...
        validate: input =>
          input.match(/\w+/) ? true : 'please enter a subject'
      },
      {
        type: 'suggest',
        name: 'contact',
        ...
        validate: input =>
          input.match(/\w+/) ? true : 'please enter a contact method'
      }
    ]);
